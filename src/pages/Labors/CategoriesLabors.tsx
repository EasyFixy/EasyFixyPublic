                    ...
                    <div className="w-2/5 h-full flex flex-col gap-5" >
                        {
                            renderLabors.map((value, index) => {
                                return (
                                    <div key={index} className={`text-xl w-full h-16 shadow-1 rounded-xl flex flex-row px-[5%] max-w-[450px] justify-between items-center ${selectedLabors.includes(value.laborId) ? 'bg-[#0E7490]' : ''}`} >
                                        <p>{value.laborName}</p>
